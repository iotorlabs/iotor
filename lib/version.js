...
// those that the ano commands directly depend on. In addition, report
...
module.exports = function() {
  ...
  REPORT_DEPENDENCIES.forEach(function(name) {
    try {
      ...
      console.error(
        'Dependency %s not found, try `npm install -g ano`.',
        name
      );
    }
  });
  ...
  return [
    util.format('ano v%s (node %s)', PACKAGE.version, process.version),
    formatReport(result, '')
  ].join('\n');
};
