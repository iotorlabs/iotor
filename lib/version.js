...
// those that the ardco commands directly depend on. In addition, report
...
module.exports = function() {
  ...
  REPORT_DEPENDENCIES.forEach(function(name) {
    try {
      ...
      console.error(
        'Dependency %s not found, try `npm install -g ardco`.',
        name
      );
    }
  });
  ...
  return [
    util.format('ardco v%s (node %s)', PACKAGE.version, process.version),
    formatReport(result, '')
  ].join('\n');
};
