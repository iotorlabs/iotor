...
// those that the racoon commands directly depend on. In addition, report
...
module.exports = function() {
  ...
  REPORT_DEPENDENCIES.forEach(function(name) {
    try {
      ...
      console.error(
        'Dependency %s not found, try `npm install -g racoon`.',
        name
      );
    }
  });
  ...
  return [
    util.format('racoon v%s (node %s)', PACKAGE.version, process.version),
    formatReport(result, '')
  ].join('\n');
};
