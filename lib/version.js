...
// those that the naco commands directly depend on. In addition, report
...
module.exports = function() {
  ...
  REPORT_DEPENDENCIES.forEach(function(name) {
    try {
      ...
      console.error(
        'Dependency %s not found, try `npm install -g naco`.',
        name
      );
    }
  });
  ...
  return [
    util.format('naco v%s (node %s)', PACKAGE.version, process.version),
    formatReport(result, '')
  ].join('\n');
};
