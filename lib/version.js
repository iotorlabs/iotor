...
// those that the iotor commands directly depend on. In addition, report
...
module.exports = function() {
  ...
  REPORT_DEPENDENCIES.forEach(function(name) {
    try {
      ...
      console.error(
        'Dependency %s not found, try `npm install -g iotor`.',
        name
      );
    }
  });
  ...
  return [
    util.format('iotor v%s (node %s)', PACKAGE.version, process.version),
    formatReport(result, '')
  ].join('\n');
};
