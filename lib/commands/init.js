...
var initAnoJson = require('../command')('bin/alm', 'init', 'ano-lm');

var init = co.wrap(function*(argv, options, cli) {
  cli.insight.track('ano', 'init');
  ...
  var result = yield initAnoJson(argv, options);
  process.chdir(process.cwd());

  if (!result) return;

  var data = fs.readJsonSync(path.resolve(output, 'ano.json'));
