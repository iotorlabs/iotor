...
Arduino.prototype.init = function (options) {
  ...
  this.settings = new Settings(this, path.resolve(options.prefs || '.anous'));
...
Arduino.prototype.populate = function () {
  ...
  var data = _.assign.apply(null, _.map([this.getPlatformPreferences(), this.getBoardPreferences(), this.context], function (prefs) {
  ...
  return _.transform(data, function (result, value, key) {
    key = key.replace(/\./g, '_');
    result[key] = value = transform(value);
    // decode receipt pattern only, not tools pattern
    if (_.endsWith(key, '_pattern') && value) {
      var command = decmd(value, _.startsWith(key, 'recipe')); // intercept make command
